...
class SIPMessage:
    ...
    def split_address(self, address):
        debug('INFO', f'{self.__class__.__name__}.{inspect.stack()[0][3]} with address {address}')
        if address.find("[", 0, 2) == -1:
            debug('DEBUG', f'{self.__class__.__name__}.{inspect.stack()[0][3]} IPv4')
            if address.find(":", 0, len(address)) == -1:
                return address, 5060
            else:
                return address.split(':')
        else:
            debug('DEBUG', f'{self.__class__.__name__}.{inspect.stack()[0][3]} IPv6')
            index = address.find("]", 0, len(address))
            if index == -1:
                debug('DEBUG', f'{self.__class__.__name__}.{inspect.stack()[0][3]} Something wrong')
                return "", ""
            else:
    ...
    def parse_header(self, header: str, data: str) -> None:
        if header == "Via":
            for d in data:
                ...
                debug('DEBUG', f'{self.__class__.__name__}.{inspect.stack()[0][3]} _address {info[1]}')
    ...
    def parse_body(self, header: str, data: str) -> None:
        debug('INFO', f'{self.__class__.__name__}.{inspect.stack()[0][3]} start')
        ...
        if self.headers['Content-Type'] == 'application/sdp':
            ...
            if header == "v":
                ...
                # sometimes the <connection-address> can be empty. for this remove
                # the last space before split.
                ...
                if len(d) == 3:
                    if d[1] == "IP6":
                        address_data, address_count, ttl = ((d[2].split('/') + [1])[:2] + [None])[:3]
                    else:
                        address_data, ttl, address_count = (d[2].split('/') + [None] + [1])[:3]
                    self.body[header].append({
                        'network_type': d[0],
                        'address_type': d[1],
                        'address': address_data,
                        'ttl': ttl,
                        'address_count': 1 if not address_count else address_count
                    })
            elif header == "b":
                # SDP 5.8 Bandwidth
                # b=<bwtype>:<bandwidth>
    ...
    def parse_raw_header(headers_raw: List[bytes],
        ...
        debug('INFO', f'SIPMessage.parseRawHeader start (staticmethod)')
    ...
    def parse_raw_body(body: bytes,
                       handle: Callable[[str, str], None]) -> None:
        debug('INFO', f"SIPMessage.parseRawBody start (staticmethode)")
        if len(body) > 0:
...
class SIPClient:
    ...
    def send_nat_hole_push(self, address: str, port: int):
        debug('INFO', f'{self.__class__.__name__}.{inspect.stack()[0][3]} called from '
                      f'{inspect.stack()[1][0].f_locals["self"].__class__.__name__}.{inspect.stack()[1][3]} '
                      f'--> message server {address} port {port}')
        self.out.sendto(b'0', (address, port))
    ...
    def recv(self) -> None:
        ...
        while self.NSD:
            ...
            try:
                ...
                if raw != b'\x00\x00\x00\x00':
                    try:
                        ...
                        debug('DEBUG',
                              f'{self.__class__.__name__}.{inspect.stack()[0][3]} Error on header parsing: {ex}'
                ...
                if "SIP Version" in str(e):
                    ...
                    debug('DEBUG', f"{self.__class__.__name__}.{inspect.stack()[0][3]} SIPParseError in SIP.recv: "
                                   f"{type(e)}, {e}")
            ...
            self.s.setblocking(True)
            self.recvLock.release()
        debug('DEBUG', f"{self.__class__.__name__}.{inspect.stack()[0][3]} End")
    ...
    def parse_message(self, message: SIPMessage) -> None:
        ...
        if message.type != SIPMessageType.MESSAGE:
            if message.status == SIPStatus.OK:
                ...
                debug('INFO', "TODO: Add 500 Error on Receiving SIP Response:\r\n" +
            ...
            try:
                ...
                debug('DEBUG', 'BYE Answer failed falling back to server as target')
                self.send_message(response)
            ...
            debug('DEBUG', "TODO: Add 400 Error on non processable request")
    ...
    # Please be aware that the OK message can have two different recipients.
    # When the invitation is running, the connected SIP server is the recipient.
    # When the BYE request gets this response, the message is for the calling client.
    ...
    def gen_ringing(self, request: SIPMessage) -> str:
        debug('INFO', f'{self.__class__.__name__}.{inspect.stack()[0][3]} called from '
                      f'{inspect.stack()[1][0].f_locals["self"].__class__.__name__}.{inspect.stack()[1][3]} start')
        tag = self.gen_tag()
        reg_request = "SIP/2.0 180 Ringing\r\n"
        reg_request += self._gen_response_via_header(request)
        reg_request += f"From: {request.headers['From']['raw']}\r\n"
        # reg_request += f"From: {request.headers['From']['raw']};tag=" + \
        #               f"{request.headers['From']['tag']}\r\n"
        reg_request += f"To: {request.headers['To']['raw']};tag={tag}\r\n"
        reg_request += f"Call-ID: {request.headers['Call-ID']}\r\n"
        reg_request += f"CSeq: {request.headers['CSeq']['check']} " + \
                       f"{request.headers['CSeq']['method']}\r\n"
        reg_request += f"Contact: {request.headers['Contact']}\r\n"
        # TODO: Add Supported
        reg_request += f"User-Agent: pyVoIP {pyVoIP.__version__}\r\n"
        reg_request += f"Allow: {(', '.join(pyVoIP.SIPCompatibleMethods))}\r\n"
    ...
    def gen_answer(self, request: SIPMessage, sess_id: str,
        ...
        reg_request += f"CSeq: {request.headers['CSeq']['check']} " \
                       f"{request.headers['CSeq']['method']}\r\n"
        reg_request += f"Contact: " \
                       f"<sip:{self.username}@{self.get_my_ip()}:{self.get_my_port()}>\r\n"
        # TODO: Add Supported
        reg_request += f"User-Agent: pyVoIP {pyVoIP.__version__}\r\n"
        reg_request += f"Allow: {(', '.join(pyVoIP.SIPCompatibleMethods))}\r\n"
        reg_request += "Content-Type: application/sdp\r\n"
        reg_request += f"Content-Length: {len(body)}\r\n\r\n"
        reg_request += body
    ...
    def invite(self, number: str, ms: Dict[int, Dict[str, 'RTP.PayloadType']],
        ...
        debug('DEBUG', f"{self.__class__.__name__}.{inspect.stack()[0][3]} <-- received Message invite 1\n----\n"
                       f"{response.raw}\n----\n")
        ...
        while ((response.status != SIPStatus(401) and
            ...
            response = SIPMessage(self.s.recv(8192))
            debug('DEBUG', f"{self.__class__.__name__}.{inspect.stack()[0][3]} <-- received Message invite 2\n----\n"
                           f"{response.raw}\n----\n")

        if (response.status == SIPStatus(100) or
                response.status == SIPStatus(180)):
            return SIPMessage(invite.encode('utf8')), call_id, sess_id
        debug('DEBUG', f"Received Response: {response.summary()}")
        ack = self.gen_ack(response)
        self.send_message(ack)
        debug('DEBUG', "Acknowledged")
        ...
        invite = self.gen_invite(number, str(sess_id), ms, sendtype, branch,
                                 call_id)
    ...
    def deregister(self) -> bool:
        ...
        if ready[0]:
            ...
            debug('DEBUG',
                  f"{self.__class__.__name__}.{inspect.stack()[0][3]} "
                  f"<-- received Message deregister 1\n----\n{resp}\n----\n")
        else:
        ...
        if response.status == SIPStatus(401):
            ...
            if ready[0]:
                ...
                debug('DEBUG',
                      f"{self.__class__.__name__}.{inspect.stack()[0][3]} "
                      f"<-- received Message deregister 2\n----\n{resp}\n----\n")
                response = SIPMessage(resp)
                if response.status == SIPStatus(401):
    ...
    def register(self) -> bool:
        debug('INFO', f'{self.__class__.__name__}.{inspect.stack()[0][3]} called from '
                      f'{inspect.stack()[1][0].f_locals["self"].__class__.__name__}.{inspect.stack()[1][3]} start')
        self.recvLock.acquire()
        first_request = self.gen_first_response()
        self.send_message(first_request)
        ...
        if len(response.headers['Via']) > 0 and 'received' in response.headers['Via'][0] \
            ...
            debug('DEBUG', f"{self.__class__.__name__}.{inspect.stack()[0][3]} after received Message register 1"
                           f" received {self.sip_client_public.get_address()} "
                           f"rport {self.sip_client_public.get_port()}")

        if response.status == SIPStatus.TRYING:
            response = SIPMessage(self.s.recv(8192))
            debug('DEBUG',
                  f"{self.__class__.__name__}.{inspect.stack()[0][3]} "
                  f"<-- received Message register 2\n----\n{response.raw}\n----\n")
        ...
        if response.status == SIPStatus(407):
            # Proxy Authentication Required
            # TODO: implement
            debug('DEBUG', 'Proxy auth required')
    ...
    def subscribe(self, lastresponse: SIPMessage) -> None:
        ...
        response = SIPMessage(self.s.recv(8192))
        debug('DEBUG',
              f"{self.__class__.__name__}.{inspect.stack()[0][3]} <-- received Message subscribe\n----\n {response.raw}\n----\n")
        debug('DEBUG', f'Got response to subscribe: {response.heading}')
