...
class Connection:
    ...
    def check_ip_and_type(self, address) -> bool:
        try:
            debug(f"{address} as ip {gethostbyname(address)}")
            ip = ipaddress.ip_address(gethostbyname(address))
            self.ip = address
            if isinstance(ip, ipaddress.IPv4Address):
                self.ip_type = 'IPv4'
                debug(f"{address} is an IPv4 address")
                return True
            elif isinstance(ip, ipaddress.IPv6Address):
                self.ip_type = 'IPv6'
                debug(f"{address} is an IPv6 address")
                return True
        except ValueError:
            debug(f"{address} is an invalid IP address")
            return False
